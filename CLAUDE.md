...
This is **cobs** - a C library template/scaffold project designed to be cloned and migrated to create new C libraries. The workflow is:
...
./CobsUnitTest       # Run test executable directly
```
...
- **include/cobs/** - Public API headers (what users #include)
  - `cobs.h` - Main API header
  - `cobs_types.h` - Common types/definitions

- **src/** - Implementation files (not exposed to users)
  - `cobs.c` - Core implementation
  - `cobs_int.c` - Internal utilities (for helpers not in public API)
...
- **cmake/** - CMake modules
  - `cobsConfig.cmake.in` - Package config template (enables find_package())
...
- CMake package config generation (enables `find_package(cobs)`)
